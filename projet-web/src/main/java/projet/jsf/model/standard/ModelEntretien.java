...
public class ModelEntretien implements Serializable {
	...
	// Pour g�rer les cultures concern�es par l'entretien
	...
	/**
	 * Charge la liste selon les crit�res de recherche et de filtrage.
	 */
	...
	/**
	 * Charge les cultures concern�es par l'entretien courant.
	 */
	private void chargerCulturesConcernees() {
		if (courant != null && courant.getId() != null) {
			try {
				...
				UtilJsf.messageError("Erreur lors du chargement des cultures concern�es : " + e.getMessage());
	...
	public String actualiserCourant() {
		if (courant != null && courant.getId() != null) {
			...
			if (dto == null) {
				UtilJsf.messageError("L'entretien demand� n'existe pas.");
				return "liste";
			} else {
	...
	public String validerMiseAJour() {
		try {
			...
			if (courant.getId() == null) {
				...
				UtilJsf.messageInfo("Entretien cr�� avec succ�s !");
			} else {
				serviceEntretien.modifier(mapper.map(courant));
				idEntretien = courant.getId();
				UtilJsf.messageInfo("Entretien modifi� avec succ�s !");
			}

			// Gestion des cultures concern�es
			if (culturesConcerneesIds != null) {
				gererCulturesConcernees(idEntretien);
			}

			// Invalide le cache local pour refl�ter les changements
			liste = null;
			return "liste";
	...
	/**
	 * G�re l'ajout et la suppression des associations culture-entretien.
	 */
	private void gererCulturesConcernees(int idEntretien) throws ExceptionValidation {
		...
		// Supprimer les associations qui ne sont plus s�lectionn�es
	...
	public String supprimer(Entretien item) {
		try {
			...
			UtilJsf.messageInfo("Entretien supprim� avec succ�s !");
		} catch (ExceptionValidation e) {
			UtilJsf.messageError(e.getMessage());
		} catch (Exception e) {
	...
	/**
	 * R�initialise tous les crit�res de recherche et de filtrage.
	 */
	...
	/**
	 * Active/d�sactive le filtre par p�riode.
	 */
	...
	/**
	 * Retourne le nombre d'entretiens affich�s (apr�s filtrage).
	 */
	...
	/**
	 * Retourne les cultures concern�es par un entretien pour l'affichage.
	 */
